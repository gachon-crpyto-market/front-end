...
import { TickCandle } from "../../components/chart/stock/tick_candle";
...
export const StockChart: React.FC<Props> = ({ width, height }) => {
  ...
  const [data, setData] = useState<frontStockQuery>(generateDummyData(50));
  const loading = false; // 더미 데이터를 사용할 경우 로딩 상태는 항상 false로 가정
  ...
  const [ticks, setTicks] = useState<frontStockQuery>(generateDummyData(50));
  ...
  // const stockArray: any[] | undefined = [];
  stockData?.forEach((item) => stockArray.push(Object.values(item)));
